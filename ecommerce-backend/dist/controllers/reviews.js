...
export const addReview = tryCatch(async (req, res, next) => {
    ...
    if (!ChangeProduct) {
        return next(new ErrorHandler(`No product found with ID ${productId}`, 404));
    }
...
export const getAllReviewsWithPagination = tryCatch(async (req, res, next) => {
    const { productId } = req.params;
    const limit = 6; // Max 6 reviews per page
    const page = parseInt(req.query.page) || 1; // Get the page number from the query, default to page 1
    if (!productId) {
    ...
    if (!productReviews) {
        return next(new ErrorHandler(`No reviews found for product with ID ${productId}`, 404));
    }
    // Sort reviews by reviewDate in descending order (newest first)
    const sortedReviews = productReviews.reviewsList.sort((a, b) => b.reviewDate.getTime() - a.reviewDate.getTime());
...
export const deleteReview = tryCatch(async (req, res, next) => {
    ...
    if (totalReviews > 0) {
        const totalRatings = productReviews.reviewsList.reduce((sum, rev) => sum + rev.rating, 0);
        ChangeProduct.rating = Math.max(1, Math.min(5, totalRatings / totalReviews)); // Ensure rating is within 1 to 5
    }
    else {
...
export const getUserReviews = tryCatch(async (req, res, next) => {
    ...
    const userReviews = productReviews.reviewsList.filter((review) => review.userId === userId);
    if (userReviews.length === 0) {
        return next(new ErrorHandler(`No reviews found for user with ID ${userId} on product with ID ${productId}`, 404));
    }
    return res.status(200).json({
        success: true,
        reviews: userReviews,
