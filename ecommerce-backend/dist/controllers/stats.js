...
export const getDashboardData = tryCatch(async (req, res, next) => {
    ...
    if (myCache.get(key)) {
        ...
        const latestFourTransactionPromise = Order.find({})
            ...
            .limit(4);
        const [ProductThisMonth, UserThisMonth, OrderThisMonth, ProductLastMonth, UserLastMonth, OrderLastMonth, totalProductCount, totalUserCount, totalOrders, OrderLastSixMonth, productCategories, femaleUserCount, latestFourTransaction,] = await Promise.all([
        ...
        //calculate revenue
        const revenueThisMonth = OrderThisMonth.reduce((total, order) => total + (order.total || 0), 0);
        const revenueLastMonth = OrderLastMonth.reduce((total, order) => total + (order.total || 0), 0);
        //calculate percentages
        const ChangePercentage = {
            revenue: calculatePercentage(revenueThisMonth, revenueLastMonth),
            product: calculatePercentage(ProductThisMonth.length, ProductLastMonth.length),
        ...
        const revenue = totalOrders.reduce((total, order) => total + (order.total || 0), 0);
        const counts = {
            revenue,
            user: totalUserCount,
...
export const getPieChart = tryCatch(async (req, res, next) => {
    ...
    if (myCache.has(key)) {
        ...
        const [ProcessingOrdersCount, ShippedOrdersCount, DeliveredOrdersCount, productCategories, totalProductCount, ProductsoutOfStock, revenueDistribution, allUsers, AdminUser, customerUser,] = await Promise.all([
        ...
        const GrossIncome = revenueDistribution.reduce((prev, order) => prev + (order.total || 0), 0);
        const discount = revenueDistribution.reduce((prev, order) => prev + (order.discount || 0), 0);
        const productionCost = revenueDistribution.reduce((prev, order) => prev + (order.shippingCharges || 0), 0);
        const burntCost = revenueDistribution.reduce((prev, order) => prev + (order.tax || 0), 0);
        const marketingCost = Math.round(GrossIncome * (30 / 100));
        const netMargin = GrossIncome - discount - productionCost - burntCost - marketingCost;
...
export const getBarChart = tryCatch(async (req, res, next) => {
    ...
    if (myCache.has(key)) {
        BarChart = JSON.parse(myCache.get(key));
    }
    else {
        //intializing Dates
        const today = new Date();
...
export const getLineChart = tryCatch(async (req, res, next) => {
    ...
    if (myCache.has(key)) {
        LineChart = JSON.parse(myCache.get(key));
    }
    else {
        //intializing Dates
        const today = new Date();
